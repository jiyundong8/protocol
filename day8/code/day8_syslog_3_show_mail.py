...
from day8.code.day8_syslog_1_create_db import Syslog, engine
from sqlalchemy import func
from day8.code.tools.day8_pygal_pie import pygal_pie
from day8.code.tools.smtp_send_mail_img import qyt_smtp_img
...
# 每个元素需要包含: name、log_count、percent
severity_level_count_html_list = []

for i in range(len(severity_level_name_list)):
    name = severity_level_name_list[i]
    count = severity_level_count_list[i]
...
# ★★★★★★ 参考目标结构如下 ★★★★★★
# [{'name': 'INFO', 'log_count': 10, 'percent': 50.0}, ...]
# ★★★★★★ 在这里编写循环，把统计结果整理到 severity_level_count_html_list 中 ★★★★★★
...
# 每个元素需要包含: ip、log_count、percent
device_ip_count_html_list = [
    {
        'ip': ip,
        'log_count': count,
        'percent': round(count / deice_log_total * 100, 2) if deice_log_total != 0 else 0
    }
...
# ★★★★★★ 参考目标结构如下 ★★★★★★
# [{'ip': '192.168.1.1', 'log_count': 20, 'percent': 66.67}, ...]
# ★★★★★★ 在这里编写循环，把统计结果整理到 device_ip_count_html_list 中 ★★★★★★
