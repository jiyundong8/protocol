...
from day7.code.day7_1_create_db import Router, engine

# 导入正课中的netmiko代码
from day7.code.tools.ssh_client_netmiko import netmiko_show_cred
...
async def async_netmiko(task_id, ip, username, password, cmds_list):
    ...
    result = await loop.run_in_executor(None, netmiko_show_cred, ip, username, password, cmds_list)
...
for router in all_routers:
    netmiko_show_cred(router.ip, router.username, router.password, "show run")